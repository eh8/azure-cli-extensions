...
# Monitoring parameters are inherited from base CLI
    ...
    with self.argument_context("aks sreclaw enable") as c:
        c.argument(
            "resource_group_name",
            options_list=["--resource-group", "-g"],
            help="Name of resource group.",
    ...
    with self.argument_context("aks sreclaw create") as c:
        ...
        c.argument(
            "namespace",
            options_list=["--namespace"],
            help="The Kubernetes namespace where the OpenClawInstance will be created.",
            required=True,
        )
        c.argument(
            "alias",
            options_list=["--alias"],
            help="User alias for the OpenClawInstance (used as the resource name and user alias).",
            required=True,
        )
        c.argument(
            "api_key",
            options_list=["--api-key"],
            help="API key for the gateway token. Falls back to API_KEY environment variable if not provided.",
        )
        c.argument(
            "api_base",
            options_list=["--api-base"],
            help="API base URL for the gateway. Falls back to API_BASE environment variable if not provided.",
        )

    with self.argument_context("aks sreclaw connect") as c:
        c.argument(
            "resource_group_name",
            options_list=["--resource-group", "-g"],
            help="Name of resource group.",
        ...
        c.argument(
            "alias",
            options_list=["--alias"],
            help="User alias. The service 'openclaw-<alias>' in namespace 'aksclaw-<alias>' will be port-forwarded.",
            required=True,
        )
        c.argument(
            "local_port",
            ...
            type=int,
            help="Local port to listen on. Defaults to 18789.",
            default=18789,
        )
